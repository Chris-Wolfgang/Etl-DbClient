...
param(
    [Parameter()]
    [string]$Repository = "Chris-Wolfgang/Wolfgang.Etl.DbClient"
)
...
if ($Repository -eq "Chris-Wolfgang/Wolfgang.Etl.DbClient" -or -not $Repository) {
    ...
    try {
        ...
        if ($Repository -eq "Chris-Wolfgang/Wolfgang.Etl.DbClient") {
