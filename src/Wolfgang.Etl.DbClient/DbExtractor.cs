...
#pragma warning restore MA0051
    ...
    private void ApplyServerPaging(string commandText, SqlMapper.IDynamicParameters? param, out string pagedCommandText, out SqlMapper.IDynamicParameters? pagedParam)
    {
        ...
        // Reject a collision before either branch adds anything. This has to sit here rather than
        // in one of the branches: EtlParameterSet can detect it itself, but the DynamicParameters
        // branch cannot - Add() there silently overwrites, so the caller's value would disappear
        // without a word. Checking the caller's own dictionary covers both routes identically.
        if (_parameters is not null)
        {
            foreach (var generated in new[] { "@PageOffset", "@PageLimit" })
            {
                if (_parameters.ContainsKey(generated))
                {
                    throw new InvalidOperationException
                    (
                        $"Parameter '{generated}' was supplied in the parameters dictionary and is " +
                        "also generated by server-side paging, so it cannot be applied twice. " +
                        $"Either remove '{generated}' from the dictionary and let paging supply " +
                        "it, or clear ServerOffset and ServerLimit and page through the command " +
                        "text yourself."
                    );
                }
            }
